...
class InvariantPointAttention(nn.Module):
    ...
    def forward(
        ...
        # print(z[0].shape)
        # im = plt.imshow(z[0][0,:,:,-1].float().detach().cpu().numpy())
        # plt.colorbar(im)
        # plt.show()
        # print(self.linear_b.weight.shape)
        # print(self.linear_b.weight[:,-1])
        # import copy
        # z2 = copy.deepcopy(z)
        # z2[0][...,-1] = 100.0
        # print(z[0][0,99,100,-1])
        # print(z[0][3,:,:,-1])
        # print(z2[0][0,99,100,-1])
        # raise OSError
        # print(z[0].shape)
        # z[0][...,-1] = 10.0
        # z[0][...,-1] = 100.0
        ...
        # [*, N_res, N_res, H]
        b = self.linear_b(z[0])
        # b = self.linear_b(z2[0])
        # b = self.linear_b(z[0]) + (100.0 * self.linear_b.weight[:,-1])[None,None,None,:]
        # print(b)
        # print(b)
        # print(b.shape)
        ...
        x_wt = self.linear_b.weight[:,-1]
        x_sq_wt = head_weights[0,0,0,:,0] * self.no_qk_points
        cs = x_wt / (2.0 * x_sq_wt)
        # print(cs.mean())
        # print(x_sq_wt.mean())
        # raise OSError
...
class StructureModule(nn.Module):
    ...
    def forward(
        ...
        # print(f'Max memory 2: {utilize_gpu:.2f}GB')
        for i in range(self.no_blocks):
            for_t = time.time() * 1000
            z = torch.cat(
                (
                    z_initial,
