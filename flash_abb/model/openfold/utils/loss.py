...
from flash_abb.model.openfold.np import residue_constants
from flash_abb.model.openfold.utils import feats
...
def compute_fape(
    ...
    Returns:
        [*] loss tensor
    """
    # num_samples = min(pred_positions.shape[-2], 100)
    num_samples = pred_positions.shape[-2]
    rand_idx = torch.multinomial(torch.ones((pred_positions.shape[-2],)), num_samples=num_samples)
    pred_positions = pred_positions[...,rand_idx,:]
    target_positions = target_positions[...,rand_idx,:]
    positions_mask = positions_mask[...,rand_idx]
    ...
    # error_dist = torch.zeros_like(local_target_pos).sum(dim=-1)
    # error_dist = torch.sqrt(
    #         # torch.sum((pred_positions[..., :, None, :] - target_positions[..., None, :, :]) ** 2, dim=-1) + eps
    #         torch.sum((pred_positions[..., None, :, :] - target_positions[..., :, None, :]) ** 2, dim=-1) + eps
    # )

    if l1_clamp_distance is not None:
        if cdr_mask is not None and l1_clamp_distance_large is not None:
            cdr_mask = cdr_mask[...,rand_idx]
...
def between_residue_bond_loss(
    ...
    Returns:
      Dict containing:
        ...
        * 'per_residue_violation_mask': mask denoting all residues with violation
            present.
    """
    if pred_atom_positions.size(-3) != pred_atom_mask.size(-2):
        raise ValueError(
            f"Expected pred_atom_positions.size(-3) and pred_atom_mask.size(-2) to both be equal to the chain length. Got {pred_atom_positions.size(-3)=} and {pred_atom_mask.size(-2)=}"
        )
    if pred_atom_positions.size(-2) != pred_atom_mask.size(-1):
        raise ValueError(
            f"Expected pred_atom_positions.size(-2) and pred_atom_mask.size(-1) to be equal to eachother with value 14 or 37. Got {pred_atom_positions.size(-2)=} and {pred_atom_mask.size(-1)=}"
        )

    # Get the positions of the relevant backbone atoms.
    this_ca_pos = pred_atom_positions[..., :-1, 1, :]  # (B, N-1, 3) <- (B, N, 14, 3)
    this_ca_mask = pred_atom_mask[..., :-1, 1]  # (B, N-1)    <- (B, N, 14)
    this_c_pos = pred_atom_positions[..., :-1, 2, :]  # (B, N-1, 3) <- (B, N, 14, 3)
    this_c_mask = pred_atom_mask[..., :-1, 2]  # (B, N-1)    <- (B, N, 14)
    next_n_pos = pred_atom_positions[..., 1:, 0, :]  # (B, N-1, 3) <- (B, N, 14, 3)
    next_n_mask = pred_atom_mask[..., 1:, 0]  # (B, N-1)    <- (B, N, 14)
    next_ca_pos = pred_atom_positions[..., 1:, 1, :]  # (B, B-1, 3) <- (B, N, 14, 3)
    next_ca_mask = pred_atom_mask[..., 1:, 1]  # (B, N-1)    <- (B, N, 14)
    has_no_gap_mask = (
        residue_index[..., 1:] - residue_index[..., :-1]
    ) == 1.0  # (B, N-1)
    ...
    mask = this_c_mask * next_n_mask * has_no_gap_mask  # (B, N-1)
    ...
    ca_c_n_cos_angle = torch.sum(c_ca_unit_vec * c_n_unit_vec, dim=-1)  # (B,N-1)
    ...
    mask = this_ca_mask * this_c_mask * next_n_mask * has_no_gap_mask  # (B, N-1)
    ...
    c_n_ca_cos_angle = torch.sum((-c_n_unit_vec) * n_ca_unit_vec, dim=-1)  # (B, n-1)
    ...
    mask = this_c_mask * next_n_mask * next_ca_mask * has_no_gap_mask  # (B, N-1)
...
def violation_loss_bondlength(violations: Dict[str, torch.Tensor]) -> torch.Tensor:
    return violations["between_residues"]["bonds_c_n_loss_mean"].mean()


def violation_loss_bondangle(violations: Dict[str, torch.Tensor]) -> torch.Tensor:
    loss = (
        violations["between_residues"]["angles_ca_c_n_loss_mean"].mean()
        + violations["between_residues"]["angles_c_n_ca_loss_mean"].mean()
    )
    return loss


def violation_loss_clash(
    violations: Dict[str, torch.Tensor],
    atom14_atom_exists: torch.Tensor,
    **kwargs,
) -> torch.Tensor:
    ...
    lengths = atom14_atom_exists[:, :, 0].sum(dim=1)  # (B, )
    l_clash = torch.sum(l_clash / lengths)
    return l_clash


def violation_loss_clash_alphafold_multimer(
    violations: Dict[str, torch.Tensor],
    atom14_atom_exists: torch.Tensor,
    eps=1e-6,
    **kwargs,
) -> torch.Tensor:
    assert violations["between_residues"]["clashes_per_atom_loss_sum"].ndim == 3
    assert violations["within_residues"]["per_atom_loss_sum"].ndim == 3

    clashes = torch.cat(
        (
            violations["between_residues"]["clashes_per_atom_loss_sum"].flatten(),
            violations["within_residues"]["per_atom_loss_sum"].flatten(),
        )
    )

    masked_clashes = clashes[clashes > 0]
    if masked_clashes.size(0) > 0.0:
        l_clash = masked_clashes.mean()
    else:
        l_clash = torch.tensor(0.0)
    return l_clash
...
def compute_renamed_ground_truth(
    ...
    num_samples = min(atom14_pred_positions.shape[-2], 100)
    rand_idx = torch.multinomial(torch.ones((atom14_pred_positions.shape[-2],)), num_samples=num_samples)
    pred_dists = torch.sqrt(
        ...
        + torch.sum(
            (
                atom14_pred_positions[..., None, :, None, :]
                # - atom14_pred_positions[..., None, :, None, :, :]
                # - atom14_pred_positions[..., None, :10, None, :, :]
                - atom14_pred_positions[..., None, rand_idx, None, :, :]
            )
            ** 2,
            dim=-1,
        )
    ...
    gt_dists = torch.sqrt(
        ...
        + torch.sum(
            (
                ...
                # - atom14_gt_positions[..., None, :, None, :, :]
                - atom14_gt_positions[..., None, rand_idx, None, :, :]
            )
            ** 2,
            dim=-1,
        )
    ...
    alt_gt_dists = torch.sqrt(
        ...
        + torch.sum(
            (
                ...
                # - atom14_alt_gt_positions[..., None, :, None, :, :]
                - atom14_alt_gt_positions[..., None, rand_idx, None, :, :]
            )
            ** 2,
            dim=-1,
        )
    ...
    mask = (
        ...
        atom14_gt_exists[..., None, :, None]
        * atom14_atom_is_ambiguous[..., None, :, None]
        * atom14_gt_exists[..., None, rand_idx, None, :]
        * (1.0 - atom14_atom_is_ambiguous[..., None, rand_idx, None, :])
    )
...
def final_output_backbone_loss(output, batch):
    """
    Extra loss used in Brennans model.

    It applies the backbone FAPE loss and angle loss (without a norm term) to the
    final layer.
    """
    bb_loss = backbone_loss(
        traj=output["frames"][-1].unsqueeze(0),
        **batch,
    )

    angle_loss = supervised_chi_loss(
        output["angles"][-1].unsqueeze(0),
        output["unnormalized_angles"][-1].unsqueeze(0),
        **{**batch, **{"chi_weight": 1.0, "angle_norm_weight": 0.0}},
    )
    return bb_loss + angle_loss


def extract_backbone_coordinates(positions):
    """(b, n, 14/37, 3) -> (b, n * 4, 3)"""
...
def cross_chain_loss(output, batch):
    """
    todo: description
    ...
    final layer.
    """
    # there is probably a more efficient way to get the c alphas
    coords_truth = batch["atom14_gt_positions"],
    coords_prediction = output["positions"][-1],
    traj = output["frames"][-1]
    pred_aff = rigid.from_tensor_7(traj)
    pred_aff = rigid(
        rotation(rot_mats=pred_aff.get_rots().get_rot_mats(), quats=none),
        pred_aff.get_trans(),
